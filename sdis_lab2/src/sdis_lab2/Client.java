...
public class Client {
	...
	public static void main(String[] args) throws IOException {
		...
		DatagramPacket request_packet = new DatagramPacket(buf, buf.length, service_address, service_port);
		request_socket.send(request_packet);
		
		buf = new byte[256];
		request_packet = new DatagramPacket(buf, buf.length);
		request_socket.receive(request_packet);
		String answer = new String(request_packet.getData(), 0, request_packet.getLength());
	...
	private static void getArguments() throws UnknownHostException {
		System.out.println("Write your arguments as: multicast_address multicast_port oper*");
	...
	private static boolean checkArguments() {
		...
		for (int i = 0; i < argumentsFromConsole.length; i++) {
			if (argumentsFromConsole[i].equals("")) {
				System.out.println("Arguments error, expected: multicast_address multicast_port oper*");
				return false;
			}
		}
